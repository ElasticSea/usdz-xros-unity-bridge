...
xros
python3 build_scripts/build_usd.py ./build_xros --build-target visionOS --no-python --no-materialx --no-tests --no-imaging --no-usdview --no-examples --no-tutorials --build-monolithic --build-args=TBB,extra_inc=big_iron.inc
```
